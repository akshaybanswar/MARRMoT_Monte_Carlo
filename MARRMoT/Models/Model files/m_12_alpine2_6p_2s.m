function [ fluxOutput, fluxInternal, storeInternal, waterBalance ] = ...
            m_12_alpine2_6p_2s( fluxInput, storeInitial, theta, solver )
% Hydrologic conceptual model: Alpine model v2
%
...
% EA(S2,Ep(t),delta_t): evaporation
EA = evap_1;

% QSE(P(t)+QN(ddf,tt,T(t),S1,delta_t),S2,Smax): saturation excess flow
QSE = saturation_1;

% QIN(S2,tcin,Cfc*Smax): interflow
QIN = interflow_8;
...
% Model states and fluxes -------------------------------------------------    
    ...
    flux_qin(t) = QIN(tmp_sFlux(2),tcin,Cfc*Smax);
    flux_qbf(t) = QBF(tcbf,tmp_sFlux(2));
    
    % Update the stores
    store_S1(t) = S1old + (flux_ps(t) - flux_qn(t)) * delta_t;
    store_S2(t) = S2old + (flux_pr(t) + flux_qn(t) - flux_ea(t) - flux_qse(t) - flux_qin(t) - flux_qbf(t)) * delta_t;    
...
%% 6. Generate outputs
    ...
    fluxOutput.Q      = (flux_qse + flux_qbf + flux_qin) * delta_t;
