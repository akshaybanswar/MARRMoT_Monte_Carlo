function [ fluxOutput, fluxInternal, storeInternal, waterBalance ] = ...
                m_31_mopex3_8p_5s( fluxInput, storeInitial, theta, solver )
% Hydrologic conceptual model: MOPEX-3
%
...
% ET1(S2,s2max,Ep(t),delta_t): evaporation from soil moisture
ET1 = evap_7;
...
% ET2(S3,se*s3max,Ep(t),delta_t): evaporation from deeper store
ET2 = evap_7;
...
% Model setup -------------------------------------------------------------
    ...
    tmpf_S2 = @(S1,S2,S3,S4,S5) (PR(P(t),T(t),tcrit) + ...
                                 ...
                                 ET1(S2,s2max,Ep(t),delta_t) - ...
                                 Q1F(PR(P(t),T(t),tcrit) +...
                                    QN(ddf,tcrit,T(t),S1,delta_t),S2,s2max) - ...
                                 QW(tw,S2));                                % store 2 function with current flux values

    tmpf_S3 = @(S1,S2,S3,S4,S5) (QW(tw,S2) - ...
                                 ET2(S3,se*s3max,Ep(t),delta_t) - ...
                                 Q2F(QW(tw,S2),S3,s3max) - ...
                                 Q2U(tu,S3));                               % store 3 function

    tmpf_S4 = @(S1,S2,S3,S4,S5) (Q1F(P(t),S2,s2max) + ...
                                 Q2F(QW(tw,S2),S3,s3max) - ...
...
% Model solving -----------------------------------------------------------            
    ...
    [tmp_sNew,tmp_fval] = fsolve(@(eq_sys) solve_fun(...
                        eq_sys(1),eq_sys(2),eq_sys(3),eq_sys(4),eq_sys(5)),...    % system of storage equations
...
% Model states and fluxes -------------------------------------------------    
    ...
    flux_et1(t)  = ET1(tmp_sFlux(2),s2max,Ep(t),delta_t);
    flux_q1f(t)  = Q1F(flux_pr(t)+flux_qn(t),tmp_sFlux(2),s2max);
    flux_qw(t)   = QW(tw,tmp_sFlux(2));
    flux_et2(t)  = ET2(tmp_sFlux(3),se*s3max,Ep(t),delta_t);
    ...
    store_S2(t) = S2old + (flux_pr(t) + flux_qn(t) - flux_et1(t) - ...
                            flux_q1f(t) - flux_qw(t)) * delta_t;
    store_S3(t) = S3old + (flux_qw(t) - flux_et2(t) - flux_q2f(t) - ...
                            flux_q2u(t)) * delta_t;    
