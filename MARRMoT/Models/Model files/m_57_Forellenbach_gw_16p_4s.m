function [ fluxOutput, fluxInternal, storeInternal, waterBalance ] = ...
    m_57_Forellenbach_gw_16p_4s( fluxInput, storeInitial, theta, solver )
% Hydrologic conceptual model: Forellenbach (traditional bucket model)
%
% Steps
% --- Practical ---
% 0. Handle inputs
...
% % Check water balance
% if nargout == 4
%     waterBalance = checkWaterBalance(((P.*delta_t) + flux_qgw),...         % Incoming precipitation
%                                      fluxOutput,...         % Fluxes Q and Ea leaving the model
%                                      storeInternal,...      % Time series of storages ...
%                                      [S10;S20;S30;S40],...       % And initial store values to calculate delta S
%                                      tmp_q0r_old.*delta_t);  % Whether the model uses a routing scheme that
