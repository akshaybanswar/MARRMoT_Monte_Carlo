function [odeFunc,strFlux] = createOdeApprox_EE(Sold,delta_t,varargin)
%createOdeApprox_EE Returns function that approximates an ODE through
%Explicit Euler.
%
...
% (S(t+1)-S(t)) / delta_t = modelFunction(S(t))
% (S(t+1)-S(t)) / delta_t - modelFunction(S(t)) = 0
...
% Create a string that specifies the flux inputs
str_fluxIn       = sprintf('Sold(%i),',1:numEq);
str_fluxIn(end)  = [];
...
str_full = sprintf(...
    ...
    ['(S%i - Sold(%i))/delta_t - varargin{%i}(',str_fluxIn,');'],1,numEq),...% insert the proper variables in sprintf
    ']'],... 
...
% Example of 'str_full'
% @(S1,S2,S3,S4,S5) 
%       [(S1 - Sold(1))/delta_t - varargin{1}(Sold(1),Sold(2),Sold(3),Sold(4),Sold(5));
%        (S2 - Sold(2))/delta_t - varargin{2}(Sold(1),Sold(2),Sold(3),Sold(4),Sold(5));
%        (S3 - Sold(3))/delta_t - varargin{3}(Sold(1),Sold(2),Sold(3),Sold(4),Sold(5));
%        (S4 - Sold(4))/delta_t - varargin{4}(Sold(1),Sold(2),Sold(3),Sold(4),Sold(5));
%        (S5 - Sold(5))/delta_t - varargin{5}(Sold(1),Sold(2),Sold(3),Sold(4),Sold(5));]
