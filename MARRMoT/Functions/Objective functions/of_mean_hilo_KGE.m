function [val,c,w] = of_mean_hilo_KGE(obs,sim,warmup,varargin)
...
if ~isempty(varargin)
    [val1,c1,w1] = of_KGE(obs,sim,warmup,varargin{1});
    [val2,c2,w2] = of_inverse_KGE(obs,sim,warmup,varargin{2});
else
    [val1,c1,w1] = of_KGE(obs,sim,warmup);
    [val2,c2,w2] = of_inverse_KGE(obs,sim,warmup);
end
